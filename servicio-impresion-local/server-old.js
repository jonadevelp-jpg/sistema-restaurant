...
let escposUSBModule; // Módulo escpos-usb completo para acceder a findPrinter()
...
try {
  ...
  if (!USB) {
    try {
      escposUSBModule = require('escpos-usb');
      if (escposUSBModule && escposUSBModule.USB) {
        USB = escposUSBModule.USB;
        console.log('✅ USB encontrado en escpos-usb (método 4)');
      } else if (typeof escposUSBModule === 'function') {
        USB = escposUSBModule;
        console.log('✅ USB encontrado como función en escpos-usb (método 4)');
      }
      // Guardar el módulo completo para acceder a findPrinter() si está disponible
      if (escposUSBModule && escposUSBModule.findPrinter) {
        console.log('✅ escpos-usb tiene findPrinter() disponible');
      }
    } catch (moduleError) {
      console.warn('⚠️  Módulo escpos-usb no disponible:', moduleError.message);
    }
  } else {
    // Si USB ya está cargado, intentar cargar escpos-usb solo para findPrinter()
    try {
      escposUSBModule = require('escpos-usb');
      if (escposUSBModule && escposUSBModule.findPrinter) {
        console.log('✅ escpos-usb cargado para acceder a findPrinter()');
      }
    } catch (moduleError) {
      // No es crítico si no se puede cargar
    }
  }
...
function connectPrinter(type, path, ip, port) {
  try {
    ...
    if (type === 'network') {
      ...
      // Detectar si el path es un puerto COM válido (COM1-COM99) o USB002/USB003
      const isComPort = /^COM\d{1,2}$/i.test(path);
      const isUsbPort = /^USB\d+$/i.test(path);
      const isVportUsb = /^vport-usb/i.test(path);
      const isLikelyPrinterName = !isComPort && !isUsbPort && !isVportUsb;
      
      console.log(`🔌 Análisis del path:`);
      console.log(`   - Es puerto COM: ${isComPort}`);
      console.log(`   - Es puerto USB: ${isUsbPort}`);
      console.log(`   - Es vport-usb: ${isVportUsb}`);
      console.log(`   - Parece nombre de impresora: ${isLikelyPrinterName}`);
      
      // Intentar encontrar findPrinter() en múltiples lugares
      let findPrinterFn = null;
      if (USB && USB.findPrinter && typeof USB.findPrinter === 'function') {
        findPrinterFn = USB.findPrinter;
        console.log(`   - USB.findPrinter disponible: Sí (desde USB)`);
      } else if (escposUSBModule && escposUSBModule.findPrinter && typeof escposUSBModule.findPrinter === 'function') {
        findPrinterFn = escposUSBModule.findPrinter;
        console.log(`   - USB.findPrinter disponible: Sí (desde escpos-usb)`);
      } else if (escposUSBModule && escposUSBModule.USB && escposUSBModule.USB.findPrinter && typeof escposUSBModule.USB.findPrinter === 'function') {
        findPrinterFn = escposUSBModule.USB.findPrinter;
        console.log(`   - USB.findPrinter disponible: Sí (desde escpos-usb.USB)`);
      } else {
        console.log(`   - USB.findPrinter disponible: No`);
      }
      ...
      // Si parece ser un nombre de impresora (no un puerto), intentar findPrinter() primero
      if (isLikelyPrinterName && findPrinterFn) {
        console.log(`🔌 Path "${path}" parece ser un nombre de impresora, intentando findPrinter() primero...`);
        try {
          const usbDevices = findPrinterFn();
          if (usbDevices && usbDevices.length > 0) {
            console.log(`🔌 Se encontraron ${usbDevices.length} dispositivo(s) USB con findPrinter()`);
            ...
            const findMethods = [
              {
                name: 'VendorId/ProductId con escpos-usb directo',
                try: () => {
                  let vid, pid;
                  
                  // Intentar múltiples formas de obtener vid/pid
                  if (foundDev.deviceDescriptor) {
                    vid = foundDev.deviceDescriptor.idVendor;
                    pid = foundDev.deviceDescriptor.idProduct;
                  } else if (foundDev.idVendor && foundDev.idProduct) {
                    vid = foundDev.idVendor;
                    pid = foundDev.idProduct;
                  } else if (foundDev.vendorId && foundDev.productId) {
                    vid = foundDev.vendorId;
                    pid = foundDev.productId;
                  }
                  
                  if (vid && pid) {
                    console.log(`   Usando vid: ${vid} (0x${vid.toString(16)}), pid: ${pid} (0x${pid.toString(16)})`);
                    
                    // PRIORIDAD 1: Usar escposUSBModule.getDevice() con el dispositivo encontrado
                    if (escposUSBModule && escposUSBModule.getDevice && typeof escposUSBModule.getDevice === 'function') {
                      try {
                        console.log(`   Intentando escposUSBModule.getDevice(foundDev)...`);
                        const device = escposUSBModule.getDevice(foundDev);
                        if (device) {
                          console.log(`   ✅ Dispositivo creado con escposUSBModule.getDevice`);
                          // Verificar que tenga métodos necesarios
                          if (typeof device.write === 'function' || typeof device.open === 'function') {
                            console.log(`   ✅ Dispositivo tiene métodos write/open`);
                            return device;
                          } else {
                            console.log(`   ⚠️  Dispositivo creado pero verificando compatibilidad...`);
                            // Intentar crear Printer para verificar
                            try {
                              const testPrinter = new Printer(device);
                              console.log(`   ✅ Dispositivo es compatible con Printer`);
                              return device;
                            } catch (e) {
                              console.log(`   ⚠️  Dispositivo no compatible: ${e.message}`);
                            }
                          }
                        }
                      } catch (e) {
                        console.log(`   escposUSBModule.getDevice falló: ${e.message}`);
                      }
                    }
                    
                    // PRIORIDAD 2: Intentar con escposUSBModule.USB directamente (si existe)
                    if (escposUSBModule && escposUSBModule.USB) {
                      // Intentar como constructor
                      if (typeof escposUSBModule.USB === 'function') {
                        try {
                          console.log(`   Intentando new escposUSBModule.USB(${vid}, ${pid})...`);
                          const device = new escposUSBModule.USB(vid, pid);
                          // Verificar que el dispositivo tenga métodos necesarios
                          if (device && (typeof device.write === 'function' || typeof device.open === 'function')) {
                            console.log(`   ✅ Dispositivo creado con escposUSBModule.USB(vid, pid)`);
                            return device;
                          } else {
                            console.log(`   ⚠️  Dispositivo creado pero no tiene métodos write/open`);
                          }
                        } catch (e) {
                          console.log(`   escposUSBModule.USB(vid, pid) falló: ${e.message}`);
                        }
                      }
                      // Si USB es un objeto con métodos open
                      if (escposUSBModule.USB.open && typeof escposUSBModule.USB.open === 'function') {
                        try {
                          console.log(`   Intentando escposUSBModule.USB.open(${vid}, ${pid})...`);
                          const device = escposUSBModule.USB.open(vid, pid);
                          if (device && (typeof device.write === 'function' || typeof device.open === 'function')) {
                            console.log(`   ✅ Dispositivo creado con escposUSBModule.USB.open(vid, pid)`);
                            return device;
                          }
                        } catch (e) {
                          console.log(`   escposUSBModule.USB.open(vid, pid) falló: ${e.message}`);
                        }
                      }
                    }
                    
                    // PRIORIDAD 2: Intentar con el adaptador usbAdapter
                    if (usbAdapter) {
                      if (typeof usbAdapter === 'function') {
                        try {
                          const device = new usbAdapter(vid, pid);
                          console.log(`   ✅ Dispositivo creado con usbAdapter(vid, pid)`);
                          return device;
                        } catch (e) {
                          console.log(`   usbAdapter(vid, pid) falló: ${e.message}`);
                        }
                      }
                      if (usbAdapter.open && typeof usbAdapter.open === 'function') {
                        try {
                          const device = usbAdapter.open(vid, pid);
                          console.log(`   ✅ Dispositivo creado con usbAdapter.open(vid, pid)`);
                          return device;
                        } catch (e) {
                          console.log(`   usbAdapter.open(vid, pid) falló: ${e.message}`);
                        }
                      }
                    }
                    
                    // PRIORIDAD 3: Intentar con USB (puede ser el adaptador)
                    if (typeof USB === 'function') {
                      try {
                        const device = new USB(vid, pid);
                        console.log(`   ✅ Dispositivo creado con USB(vid, pid)`);
                        return device;
                      } catch (e) {
                        console.log(`   Constructor USB(vid, pid) falló: ${e.message}`);
                      }
                    }
                    
                    // PRIORIDAD 4: Intentar crear dispositivo USB usando el módulo usb de Node.js directamente
                    try {
                      const usb = require('usb');
                      if (usb && usb.findByIds) {
                        const usbDevice = usb.findByIds(vid, pid);
                        if (usbDevice) {
                          console.log(`   ✅ Dispositivo USB encontrado con usb.findByIds(${vid}, ${pid})`);
                          // Abrir el dispositivo
                          usbDevice.open();
                          // Obtener la interfaz y el endpoint
                          const iface = usbDevice.interface(0);
                          if (iface) {
                            iface.claim();
                            const endpoint = iface.endpoint(1); // Endpoint de salida típico
                            if (endpoint) {
                              // Crear un adaptador personalizado que tenga el método write
                              const customAdapter = {
                                open: () => {},
                                write: (data, callback) => {
                                ...
                                close: (callback) => {
                                  ...
                                  if (usbDevice) usbDevice.close();
                                  if (callback) callback();
                                }
                              };
                              console.log(`   ✅ Adaptador personalizado creado con método write`);
                      ...
                      console.log(`   Módulo usb no disponible o error: ${usbError.message}`);
                    }
                    
                    // PRIORIDAD 5: Intentar usar el objeto foundDev directamente con escpos.create (último recurso)
                    if (escpos && escpos.create && typeof escpos.create === 'function') {
                      try {
                        // Intentar crear dispositivo USB pasando el objeto encontrado
                        const device = escpos.create('usb', foundDev);
                        console.log(`   ✅ Dispositivo creado con escpos.create('usb', foundDev)`);
                        return device;
                      } catch (e) {
                        console.log(`   escpos.create('usb', foundDev) falló: ${e.message}`);
                      }
                    }
                    
                    throw new Error('No se pudo crear dispositivo con vid/pid usando ningún método disponible');
                  }
                  
                  throw new Error(`No se puede extraer vid/pid. Estructura del dispositivo: ${JSON.stringify(Object.keys(foundDev))}`);
                }
              },
              {
              ...
              {
                name: 'Adaptador create()',
                try: () => {
                  if (USB && typeof USB === 'object' && typeof USB.create === 'function') {
                    return USB.create(foundDev);
                  }
                  throw new Error('Adaptador no tiene create()');
                }
              },
              {
            ...
            console.log(`⚠️  No se encontraron dispositivos USB con findPrinter(), continuando con métodos de path...`);
          }
        } catch (listError) {
      ...
      // Si findPrinter() no funcionó o el path es un puerto, intentar métodos de path
      if (!usbConnected) {
      ...
      // Intentar cada método de path hasta que uno funcione
      ...
      // Método de respaldo: Si todos los métodos de path fallaron y no se intentó findPrinter() antes, intentarlo ahora
      if (!usbConnected && !isLikelyPrinterName && findPrinterFn) {
        try {
