...
interface MenuImprimibleProps {
  categorySlug?: string;
}

export default function MenuImprimible({ categorySlug }: MenuImprimibleProps) {
  ...
  async function loadCategories() {
    try {
      ...
      // Si hay categorySlug, seleccionar esa categoría
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-screen bg-[#0a0a0a]">
        <div className="text-[#d4af37] text-xl">Cargando menú...</div>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      {/* Menú imprimible - generar páginas si es platillos y tiene muchos items */}
      {(() => {
        const isPlatillos = selectedCategory.slug === 'platillos';
        ...
        const categoryImage = selectedCategory.image_url;
          ...
          return (
            ...
            >
              ...
              <div className="menu-header">
                ...
                {selectedCategory.description && isFirstPage && (
                  <p className="menu-subtitle">{selectedCategory.description}</p>
                )}
              </div>

              {/* Imagen principal - en todas las páginas si hay imagen disponible */}
              {displayImage && (
      ...
      <style jsx global>{`
        @media print {
          ...
          * {
          ...
          /* Para platillos, permitir 2 páginas */
          ...
          .menu-imprimible {
            ...
            background-image: 
              repeating-linear-gradient(
                ...
                transparent,
                transparent 25px,
                rgba(184, 134, 11, 0.08) 25px,
                rgba(184, 134, 11, 0.08) 50px
              ),
              repeating-linear-gradient(
                -45deg,
                transparent,
                transparent 25px,
                rgba(184, 134, 11, 0.05) 25px,
                rgba(184, 134, 11, 0.05) 50px
              );
            padding: 60px 50px;
            color: white;
            font-family: 'Cinzel', 'Playfair Display', serif;
